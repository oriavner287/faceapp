...
export interface AppConfig {
  ...
  apiBaseUrl: string
  // Security configuration following security-expert.md
  security: {
    ...
    biometricDataTTL: number
    rateLimiting: {
      windowMs: number
      maxRequests: number
      faceDetectionMax: number
    }
  }
...
export function getConfig(): AppConfig {
  ...
  const security = {
    ...
    biometricDataTTL: parseIntSafe(
      process.env["BIOMETRIC_DATA_TTL"],
      86400000,
      3600000
    ), // 24 hours, min 1 hour
    rateLimiting: {
      ...
      faceDetectionMax: parseIntSafe(
        process.env["RATE_LIMIT_FACE_DETECTION_MAX"],
        20,
        1,
        1000
      ),
  ...
  const videoFetching = {
    timeout: parseIntSafe(
      process.env["VIDEO_FETCH_TIMEOUT"],
      30000,
      5000,
      120000
    ), // 30s, min 5s, max 2min
    maxVideosPerSite: parseIntSafe(
      process.env["MAX_VIDEOS_PER_SITE"],
      10,
      1,
      100
    ),
  ...
  if (isProduction) {
    ...
    if (security.jwtSecret.includes("dev-")) {
      console.error(
        "⚠️  SECURITY WARNING: Using development JWT secret in production!"
      )
    }
  }
