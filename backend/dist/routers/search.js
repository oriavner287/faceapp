...
export const searchRouter = os.router({
    ...
    configure: os
        ...
        try {
            ...
            const updateResult = similarityMatchingService.updateMatchesWithThreshold(session.results, input.threshold);
            if (!updateResult.success) {
                console.error("Failed to update matches with threshold:", updateResult.error?.message);
                return {
                    success: false,
                    updatedResults: [],
                };
            ...
            sessionService.updateSessionResults(input.searchId, updateResult.matches);
            return {
                success: true,
