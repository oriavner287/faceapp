...
export const videoRouter = os.router({
    fetchFromSites: os
        ...
        .handler(async ({ input }) => {
        try {
            console.log("Fetching videos with embedding length:", input.embedding.length);
            ...
            console.log(`Fetched ${fetchResult.results.length} videos from ${fetchResult.processedSites.length} sites`);
            ...
            const downloadResult = await videoFetchingService.downloadThumbnails(fetchResult.results);
            ...
            for (const video of downloadResult.processedVideos) {
                try {
                    if (!video.localThumbnailPath) {
                        processingErrors.push(`No local thumbnail for video: ${video.title}`);
                        continue;
                    }
                    // Detect faces in thumbnail
                    const faceDetections = await faceDetectionService.detectFacesInImage(video.localThumbnailPath);
                    ...
                    for (const detection of faceDetections) {
                        const similarity = calculateCosineSimilarity(input.embedding, detection.embedding);
                    ...
                    const errorMsg = `Failed to process video ${video.title}: ${error instanceof Error ? error.message : "Unknown error"}`;
