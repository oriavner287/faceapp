...
export const faceRouter = os.router({
    processImage: os
        ...
        try {
            ...
            try {
                ...
                console.error("Failed to initialize face detection service:", initError);
                return {
                    success: false,
                    faceDetected: false,
                    searchId: "",
            ...
            let embeddingResult;
            try {
                const embeddingPromise = faceDetectionService.generateEmbedding(input.imageData);
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Face detection timeout")), 15000));
            ...
            const embeddingValidation = ValidationSchemas.validateFaceEmbedding(embeddingResult.embedding);
            if (!embeddingValidation.isValid) {
                console.error("Embedding validation failed:", embeddingValidation.errors);
                return {
                    success: false,
                    faceDetected: false,
                    searchId: "",
            ...
            const sessionResult = await sessionService.createSession(embeddingResult.embedding, SIMILARITY_CONSTRAINTS.DEFAULT_THRESHOLD);
            ...
            while (retryCount < maxRetries) {
                storeResult = await sessionService.storeSessionImage(session.id, input.imageData);
                if (storeResult.success) {
                    break;
                }
                retryCount++;
                console.warn(`Image storage attempt ${retryCount} failed:`, storeResult.error);
            ...
            if (!storeResult?.success) {
                console.error("Failed to store session image after retries:", storeResult?.error);
            ...
            console.log(`Face detected successfully. Session ID: ${session.id}, Processing time: ${processingTime}ms`);
            return {
                success: true,
                faceDetected: true,
                searchId: session.id,
    ...
    getSession: os.input(GetSessionInputSchema).handler(async ({ input }) => {
        try {
            ...
            const sessionIdValidation = ValidationSchemas.validateSearchId(input.sessionId);
            if (!sessionIdValidation.isValid) {
                return {
                    success: false,
                    error: {
                        code: "VALIDATION_ERROR",
                        message: sessionIdValidation.errors[0]?.message || "Invalid session ID",
                    },
                };
            }
    ...
    updateThreshold: os
        ...
        try {
            ...
            // Validate threshold
            const thresholdValidation = ValidationSchemas.validateSimilarityThreshold(input.threshold);
            if (!thresholdValidation.isValid) {
                return {
                    success: false,
                    error: {
                        code: "INVALID_THRESHOLD",
                        message: thresholdValidation.errors[0]?.message || "Invalid threshold",
                    },
                };
            }
            ...
            const sessionResult = sessionService.updateSessionThreshold(input.sessionId, input.threshold);
            if (!sessionResult.success || !sessionResult.data) {
                return {
                    success: false,
                    error: sessionResult.error || {
            ...
            console.log(`Updated threshold for session ${input.sessionId} to ${input.threshold}`);
            return {
                success: true,
            ...
            return {
                ...
                error: {
                    code: "INTERNAL_SERVER_ERROR",
                    message: `Failed to update threshold: ${error instanceof Error ? error.message : "Unknown error"}`,
                },
            };
        }
    }),
    deleteSession: os
        ...
        try {
            ...
            if (sessionExists.success) {
                ...
                console.log(`Attempting to delete non-existent session ${input.sessionId}`);
            }
    ...
    cleanupSessions: os.handler(async () => {
        try {
            ...
            return {
                ...
                error: {
                    code: "INTERNAL_SERVER_ERROR",
                    message: `Failed to cleanup sessions: ${error instanceof Error ? error.message : "Unknown error"}`,
                },
                timestamp: new Date().toISOString(),
            };
        }
    ...
    healthCheck: os.handler(async () => {
        ...
        try {
            ...
            const status = modelsAvailable && initializationStatus === "initialized"
                ? "healthy"
                : modelsAvailable
                    ? "degraded"
                    : "models_missing";
